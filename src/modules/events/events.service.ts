...
import { ClientUpdateEventDto } from './dto/client-update-event.dto';
...
const EVENT_TYPE_CUTOFFS: Record<string, number> = {
  'wedding': 72, // 3 days
  'corporate': 48, // 2 days
  'standard': 24, // 1 day
};
...
export class EventsService {
  ...
  async create(dto: CreateEventDto) {
    ...
    try {
      await this.auditLogs.log(result.id, AuditActorType.STAFF, 'ORDER_CREATED', {
  ...
  async importEventTree(dto: ImportEventDto) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const baseEventData: Prisma.eventsCreateInput = {
        ...
        sales_tax_pct: dto.salesTaxPct != null ? new Prisma.Decimal(dto.salesTaxPct) : null,
        sales_tax_amount: dto.salesTaxAmount != null ? new Prisma.Decimal(dto.salesTaxAmount) : null,
        headcount_est: dto.headcountEst ?? null,
      ...
      const salesTaxAmount = eventRow.sales_tax_amount ?? new Prisma.Decimal(0);
    ...
    try {
      if (result) {
        await this.auditLogs.log(result.id, AuditActorType.STAFF, 'ORDER_IMPORTED', {
          metadata: {
  ...
  async updateEventTree(id: bigint, dto: ClientUpdateEventDto) {
      ...
      if (dto.caterings) {
        ...
        for (const cat of dto.caterings) {
          ...
          for (const ord of cat.orders) {
            ...
            if (ord.items?.length) {
              const rows = ord.items.map((it, i) => {
                const qtyPerUnitDec = it.qtyPerUnit != null ? new Prisma.Decimal(it.qtyPerUnit) : new Prisma.Decimal(1);
                const componentPriceDec = it.componentPrice != null ? new Prisma.Decimal(it.componentPrice) : null;
                return {
                  event_catering_order_id: order.id,
                  position_number: i + 1,
                  item_id: BigInt(it.itemId),
        ...
        if (!ev) throw new NotFoundException('Event not found during update');

        const delivery = ev.delivery_charges ?? new Prisma.Decimal(0);
        const service = ev.service_charges ?? new Prisma.Decimal(0);
        const discount = ev.discount ?? new Prisma.Decimal(0);
        const salesTaxAmount = ev.sales_tax_amount ?? new Prisma.Decimal(0); // Note: Tax might need recalc if it depends on subtotal?
        // If tax is amount-based, it might be outdated. If pct-based, we should recalc it.

        let finalTaxPromise = new Prisma.Decimal(salesTaxAmount);
  ...
  async getEventViewById(id: bigint) {
    const ev = await this.prisma.events.findUnique({
      where: { id },
      include: {
        customer: true,
    ...
    const cutoffTime = this.getCutoffTime(ev.event_datetime, (ev as any).event_type);
    const isLocked = this.isEventLocked(ev as any);
   ...
   */
  getCutoffTime(eventDate: Date, eventType: string | null): Date {
   ...
   */
  isEventLocked(event: { is_locked: boolean, event_datetime: Date, event_type: string | null }): boolean {
